...
SCIP_DECL_READERFREE(ReaderBP::scip_free)
{
	return SCIP_OKAY;
} /*lint !e715*/
 ...
 */
SCIP_DECL_READERWRITE(ReaderBP::scip_write)
{
 ...
 */
SCIP_DECL_READERREAD(ReaderBP::scip_read) {
	*result = SCIP_DIDNOTRUN;
   ...
   SCIPdebugMessage("Start read!\n");
	...
	// read parameters
    SCIP_Real capacity, Dalpha = 0;
	int numitems;
	filedata >> numitems;
	filedata >> capacity;

	vector<SCIP_Real> mus(numitems);
	vector<SCIP_Real> bs(numitems, 0);
