...
void solve_conic_knap(
   ...
   if(Dalpha > tol){
      ...
      if(!algo_conf.is_misocp){ // PL-BC
         solve_conic_knap_approx(scip, objs,  mus,  bs,  Dalpha,  capacity,  numitems,  init_estimator, pr_param, algo_conf, summu, items_diff,  conflict,  sol_pool,  
            sol_val,  sol_ub, sol_type,  time_limit ,  target_lb);
         //SCIPdebugMessage("hist9\n");
